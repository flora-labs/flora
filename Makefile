...
HTTPS_GIT := github.com/rollchains/flora.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=flora \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=florad \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/florad ./cmd/florad
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/florad.exe ./cmd/florad
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/florad
...
draw-deps:
	...
	@goviz -i ./cmd/florad -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c flora --local -f chains.yaml
endif
...
set-testnet-configs:
	florad config set client chain-id localchain_9000-1
	florad config set client keyring-backend test
	florad config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | florad keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | florad keys add acc1 --recover`
